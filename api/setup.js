// One-time database setup: creates tables and seeds products.
// Call once after deployment: GET /api/setup?key=<SETUP_KEY>
    ...
    if (parseInt(rows[0].count) === 0) {
      for (const p of PRODUCTS) {
        await pool.query(
          ...
          [p.id, p.name, p.category, p.price, p.unit, p.stock, p.min_order_qty, p.bulk_discount, p.demand, p.image, p.description]
        );
      }
      seeded = true;
    }

    res.json({ success: true, message: `Tables created. Products seeded: ${seeded}` });
  } catch (err) {
